...
function Post({ post, user, usertype }) {
  ...
  if (isCustomer && isNew) {
    ...
    author = useDocWithCache(post.courier.path);
  } else if (!isCustomer) {
    author = useDocWithCache(post.author.path);
  }
  return (
...
function DropdownMenu({ setDropdownOpen, user, usertype, status, id }) {
